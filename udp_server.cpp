...
{
    //ホストの座標を送信
    SendXMFLOAT3(addsocket,Manager::GetScene()->GetGameObject<Player>()->m_TransForm->_Position);
}
...
void UDP_Server::ReflectOtherPlayer()
{
    ...
    {
         otherplayer->m_TransForm->_Position = _AllPlayerData[otherplayer->playerID].pos;
    }
}
...
void UDP_Server::ServerLoop() {
    ...
    while (running) {
        ...
        if(recvLen == sizeof(OtherPlayerData))
        {
            ...
            _AllPlayerData[0].pos = Manager::GetScene()->GetGameObject<Player>()->m_TransForm->_Position;
