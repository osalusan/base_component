...
public:
	...
	// インスタンス化の時に呼ぶ
	void SetBillBoard(XMFLOAT3 position, XMFLOAT3 size) { m_Position = position; m_TransForm->m_Scale = size; };
	void SetAnimation(int maxX, int maxY, int maxT) { m_Sprite.x = maxX; m_Sprite.y = maxY; m_Maxcount = maxT; }
	void SetBillBoardAnimation(XMFLOAT3 position, XMFLOAT3 size, int maxX, int maxY, int maxT) { m_Position = position; m_TransForm->m_Scale = size; m_Sprite.x = maxX; m_Sprite.y = maxY; m_Maxcount = maxT; }
