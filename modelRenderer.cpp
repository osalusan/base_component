...
void ModelRenderer::LoadMaterial( const char *FileName, MODEL_MATERIAL **MaterialArray, unsigned int *MaterialNum )
{
	...
	while( true )
	{
		...
		else if( strcmp( str, "map_Kd" ) == 0 )
		{
			...
			//テクスチャの指定を直接変える
			//if (_chengeTexture == nullptr) { strcat(materialArray[mc].TextureName, path); }
			//else { strcat(materialArray[mc].TextureName, _chengeTexture); }
			//_chengeTexture = nullptr;
		}
	}
